...
export default function FormularioActaQA({ equiposAsignados = [], formatoQa, token, onSuccess, onCancel }) {
  ...
  // Equipo desde lista técnico
  const [eqSel, setEqSel] = useState('');

  // Filas manuales (instalados / desinstalados)
  const EMPTY_EQ = { sap: '', descripcion: '', serial: '', placa: '' };
  ...
  const setMed = (k, v) => setForm(p => ({ ...p, mediciones_electricas: { ...p.mediciones_electricas, [k]: v } }));
  const setPrueba = (k, v) => setForm(p => ({ ...p, pruebas_servicio: { ...p.pruebas_servicio, [k]: v } }));

  const agregarDesdeLista = () => {
    if (!eqSel) return;
    const eq = equiposAsignados.find(e => e.id === parseInt(eqSel));
    if (!eq) return;
    setForm(p => ({ ...p, equipos_instalados: [...p.equipos_instalados, { sap: eq.material_id || '', descripcion: eq.material_descripcion || eq.descripcion || '', serial: eq.serial || '', placa: eq.placa || '' }] }));
    setEqSel('');
  };

  const agregarInst = () => {
    if (!nuevoInst.sap || !nuevoInst.descripcion || !nuevoInst.serial) { setAlert({ type: 'error', msg: 'Completa SAP, Descripción y Serie.' }); return; }
    setForm(p => ({ ...p, equipos_instalados: [...p.equipos_instalados, { ...nuevoInst }] }));
    setNuevoInst({ ...EMPTY_EQ });
  };

  const agregarDesinst = () => {
    if (!nuevoDesinst.sap || !nuevoDesinst.descripcion || !nuevoDesinst.serial) { setAlert({ type: 'error', msg: 'Completa SAP, Descripción y Serie.' }); return; }
    setForm(p => ({ ...p, equipos_desinstalados: [...p.equipos_desinstalados, { ...nuevoDesinst }] }));
    setNuevoDesinst({ ...EMPTY_EQ });
  };

  const quitarInst   = (i) => setForm(p => ({ ...p, equipos_instalados: p.equipos_instalados.filter((_,j) => j !== i) }));
  const quitarDesinst = (i) => setForm(p => ({ ...p, equipos_desinstalados: p.equipos_desinstalados.filter((_,j) => j !== i) }));
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      const pdfRes = await fetch(`/api/actas-qa/${data.id}/pdf`, { headers: { Authorization: `Bearer ${token}` } });
      if (pdfRes.ok) {
      ...
      : (
        <div style={{ overflowX: 'auto', marginTop: 10 }}>
          <table>
            <thead>
              <tr>
                {['Código SAP', 'Descripción', 'Serie', 'Placa', ''].map(h => <th key={h}>{h}</th>)}
              </tr>
            </thead>
            <tbody>
              {items.map((eq, i) => (
  ...
  const FilaManual = ({ val, set, onAdd, label }) => (
    <div style={{ marginTop: 14, padding: '12px 14px', background: 'rgba(255,255,255,0.02)', borderRadius: 10, border: '1px solid var(--border)' }}>
      ...
      <div style={{ display: 'flex', gap: 8, flexWrap: 'wrap', alignItems: 'flex-end' }}>
        {[['sap','Código SAP','100px'],['descripcion','Descripción','1fr'],['serial','Serie','140px'],['placa','Placa','110px']].map(([k, ph, w]) => (
          <div key={k} style={{ flex: w === '1fr' ? 1 : undefined, width: w === '1fr' ? undefined : w }}>
            <input value={val[k]} onChange={e => set(p => ({ ...p, [k]: e.target.value }))} placeholder={ph} />
          </div>
        ))}
    ...
    <div className="fade-in" style={{ maxWidth: 900, margin: '0 auto' }}>
      ...
      <SECTION title="Datos Generales" icon="ti-file-text">
        ...
        <div style={{ marginTop: 16 }}>
          <div style={{ fontSize: 11, fontWeight: 700, color: 'var(--text-muted)', textTransform: 'uppercase', letterSpacing: '0.06em', marginBottom: 10 }}>Tiempos (minutos)</div>
          <GRID cols={4}>
            {[['tiempo_transporte','Transporte'],['tiempo_antesala','Antesala'],['tiempo_ejecucion','Ejecución'],['tiempo_espera_claro','Espera Claro']].map(([k,l]) => (
        ...
        <div style={{ marginTop: 16 }}>
          <div style={{ fontSize: 11, fontWeight: 700, color: 'var(--text-muted)', textTransform: 'uppercase', letterSpacing: '0.06em', marginBottom: 10 }}>Equipos de medición</div>
          <GRID cols={2}>
      ...
      <SECTION title="Mediciones Eléctricas" icon="ti-bolt">
        <GRID cols={3}>
          {[['fase_neutro','Fase — Neutro (V)'],['fase_tierra','Fase — Tierra (V)'],['neutro_tierra','Neutro — Tierra (V)']].map(([k,l]) => (
            <Field key={k} label={l}>
      ...
      <SECTION title="Equipos Instalados" icon="ti-package-import">
              ...
              <select value={eqSel} onChange={e => setEqSel(e.target.value)} style={{ flex: 1 }}>
                ...
                {equiposAsignados.map(eq => (
                  <option key={eq.id} value={eq.id}>
                    {eq.serial ? `${eq.serial} — ` : ''}{eq.material_descripcion || eq.descripcion || eq.material_id} ({eq.estado})
                  </option>
                ))}
              </select>
        ...
        <FilaManual val={nuevoInst} set={setNuevoInst} onAdd={agregarInst} />
      </SECTION>

      {/* 4 · Equipos desinstalados */}
      <SECTION title="Equipos Desinstalados / Trasladados" icon="ti-package-export">
        ...
        <FilaManual val={nuevoDesinst} set={setNuevoDesinst} onAdd={agregarDesinst} />
      </SECTION>

      {/* 5 · Pruebas de servicio */}
      <SECTION title="Pruebas de Servicio" icon="ti-network">
        <GRID cols={1}>
          <Field label="Ping a oficina principal"><input value={form.pruebas_servicio.ping_central} onChange={e => setPrueba('ping_central', e.target.value)} placeholder="ms / resultado" /></Field>
          <Field label="Traceroute"><input value={form.pruebas_servicio.traceroute} onChange={e => setPrueba('traceroute', e.target.value)} placeholder="Resultado del traceroute" /></Field>
          <Field label="Versión de Firmware"><input value={form.pruebas_servicio.firmware} onChange={e => setPrueba('firmware', e.target.value)} placeholder="v1.x.x" /></Field>
        </GRID>
      </SECTION>

      {/* 6 · Observaciones y checkboxes */}
      <SECTION title="Observaciones y Cierre" icon="ti-notes">
        ...
        <div style={{ marginTop: 16, display: 'flex', gap: 20, flexWrap: 'wrap' }}>
          {[['firma_acta','Firma acta','#22C55E'],['caso_seguimiento','Caso seguimiento','#F97316'],['problemas_instalacion','Problemas instalación','#EF4444']].map(([k,l,c]) => (
